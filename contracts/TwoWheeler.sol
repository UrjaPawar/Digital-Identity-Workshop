...
contract TwoWheeler {
  ...
  function getAuth(address _beneficiary) public constant returns (bool) {
    return isAuthorised[_beneficiary];
  }
  function Validate(address _beneficiary) public returns (bool) {
    require(_beneficiary != address(0));
    ClaimHolder beneficiaryIdentity = ClaimHolder(_beneficiary);
    require(checkClaim(beneficiaryIdentity, 7));
    isAuthorised[_beneficiary] =true;
    return true;
  }
